...
<?php
	$sql = "SELECT * from tb_doc_upload Where visibility='Public' Or visibility='To a user' Or (visibility='Private' And username='$username_log')  ORDER BY id DESC limit 5";
	...
	if(strlen($string) > 80) {
		echo $textdisplaylist = '<div>'.substr($string,0,80).'...<br><pre id=skip><a href=view-doc.php?docid='.$result['id'].'&&cat='.$result['category'].'>Read More</a> | Visibility: '.$result['visibility'].'</pre></div>';
	}else{
		echo $textdisplaylist = '<div>'.$string.'<br><pre id=skip><a href=view-doc.php?docid='.$result['id'].'&&cat='.$result['category'].'>View</a> | Visibility: '.$result['visibility'].'</pre></div>';
	}
