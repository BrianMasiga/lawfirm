...
include 'config/log_connection.php'; // include the library for database connection
...
if(isset($_POST['action']) && $_POST['action'] == 'login'){ // Check the action `login`
	...
	$query			= mysql_query('SELECT * FROM tb_users WHERE username = "'.$username.'" AND password = "'.$password.'" '); // Check the table with posted credentials
